...
write.table(geodata, file = as.character(paste(Site, Date, ".csv", sep="")), col.names=TRUE,row.names=F, sep=",")
Syncfolder<-unlist(strsplit(getwd(), split="Data"))[1]
write.table(geodata, file = as.character(paste(Syncfolder,"DataArchive/" , Site, Date, ".csv", sep="")), col.names=TRUE,row.names=F, sep=",")
